...
const Work = () => {
  ...
  const [animeteCard, setAnimeteCard] = useState({ y: 0, opacity: 1 })
  const [works, setWorks] = useState([])
  const [fillterWork, setFillterWork] = useState([])

  const handleWorkFilter = (item) => {
    setActiveFilter(item)
    setAnimeteCard([{ y: 100, opacity: 0 }])

    setTimeout(() => {
      setAnimeteCard([{ y: 0, opacity: 1 }])
  ...
  return (
    <>
      ...
      <div className='app__work-filter'>
        {['All', 'Web App', 'React JS', 'Svelte', 'Next JS', 'GO', 'PHP', 'Blockchain'].map((item, index) => (
          <div
            key={index}
